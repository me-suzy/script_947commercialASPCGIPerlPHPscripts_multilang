...
// Afterburners Hack Start
  makeyesnocode("Show last title of new postings or replays on the mainpage?","showlasttitle",1);
// Afterburners Hack End
...
if ($HTTP_POST_VARS['action']=="insert") {
  ...
  $DB_site->query("INSERT INTO forum
                      (forumid,styleid,title,description,active,displayorder,parentid,
                       ...
                       styleoverride,allowratings,countposts,moderateattach,showlasttitle)
                   VALUES
...
// Afterburners Hack Start
  makeyesnocode("Show last title of new postings or replays on the mainpage?","showlasttitle",$forum[showlasttitle]);
// Afterburners Hack End
...
if ($HTTP_POST_VARS['action']=="doupdate") {
  ...
  $DB_site->query("UPDATE forum
                   SET
                     ...
                     moderateattach='$moderateattach', showlasttitle='$showlasttitle'
...
if ($action=="lasttitle") {
echo "<p align=center>This are the settings for the lasttitle hack made by <a href=\"http://www.the-afterburner.de\">Afterburner</a></p>";

  doformheader("forum","lasttitlelength");
  maketableheader("length of the title");
  $forumslasttitle1=$DB_site->query("SELECT * FROM lasttitle WHERE field=1");
  while ($forumlasttitle1=$DB_site->fetch_array($forumslasttitle1)) {
  makeinputcode("length (0-250)","lasttitlelength",$forumlasttitle1[lasttitlelength]); 
	}
  doformfooter("Update length of the title");


  doformheader("forum","lasttitleupdate");
  maketableheader("Show lasttitle");
  $forumslasttitle=$DB_site->query("SELECT forumid,showlasttitle,title FROM forum ORDER BY forumid");
  while ($forumlasttitle=$DB_site->fetch_array($forumslasttitle)) {
  makeyesnocode("<li><b>$forumlasttitle[title]</b></li>","showlasttitle[$forumlasttitle[forumid]]",$forumlasttitle[showlasttitle]);
   }
  doformfooter("Update settings");


}

if ($HTTP_POST_VARS['action']=="lasttitlelength") {
echo "<center><b>new length for the title is $lasttitlelength </b></center>";
$DB_site->query("UPDATE lasttitle SET lasttitlelength='$lasttitlelength' WHERE field=1");
}


if ($HTTP_POST_VARS['action']=="lasttitleupdate") {
echo "<center><table border=\"1\"><tr><td>Forumid</td><td>Show/Dont Show</td></tr>";
while (list($showlasttitleforumid,$setshowlasttitle)=each($showlasttitle)) {
$DB_site->query("UPDATE forum SET showlasttitle='$setshowlasttitle' WHERE forumid='$showlasttitleforumid'");
if ($setshowlasttitle==1) {
	$display="<font color=\"green\">Show LastTitle</font>";
}
if ($setshowlasttitle==0) {
	$display="<font color=\"red\">Dont Show LastTitle</font>";
}
echo "<tr><td>$showlasttitleforumid</td><td> $display</td></tr>";
}
echo "</table><br><b>done</b></center>";
}
...
// Afterburner
  echo "<p align=center><a href=\"forum.php?s=$session[sessionhash]&action=lasttitle\"><b>--> Edit the settings for the Last-Title-Hack <-- </b></a></p>";
// Afterburner
