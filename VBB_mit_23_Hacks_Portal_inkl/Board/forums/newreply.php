...
if (isset($postid)) {
  ...
  if ($postid!=0) {
    ...
    $HIDE_shown = 0; 
    if (substr($pagetext,0,6)=="[HIDE]") $pagetext = str_replace("[HIDE]"," [HIDE]",$pagetext);
    if ($session[userid] != 0) {
      if ($bbuserinfo['usergroupid']!=6) {
        $hasposted=$DB_site->query_first("SELECT userid FROM post WHERE threadid='$threadid' AND userid='$bbuserinfo[userid]' LIMIT 1");
      }
      if ($hasposted['userid'] || $bbuserinfo['usergroupid']==6) $HIDE_shown = 1; 
    }

    if ($HIDE_shown == 0) { 
      for ($i = lock_count($pagetext,"[HIDE]"); $i > 0; $i--) { 
        $lock_part1 = strpos($pagetext, "[HIDE]");
        $lock_part2 = (strpos($pagetext,"[/HIDE]")-strpos($pagetext,"[HIDE]"))+7;
        $lock_mess = substr ($pagetext, $lock_part1, $lock_part2);
        $pagetext = str_replace($lock_mess, "[COLOR=\"red\"][B][VERSTECKTER TEXT][/B][/COLOR]\nDu musst auf dieses Thema antworten, um den versteckten Text zu sehen!",  $pagetext);
      } 
    }
    // ##### [HIDE] [/HIDE] HACK End #####
    ...
    if ($postcounter++ < $maxposts) {
            ...
            if ($session[userid] != 0) {
              ...
              if ($hasposted['userid'] || $bbuserinfo['usergroupid']==6) {
                $reviewmessage = str_replace("[HIDE]","<font color='red'><b>[VERSTECKTER TEXT]:</b></font><br>",$reviewmessage);
            ...
            if ($HIDE_shown == 0) { 
              for ($i = lock_count($reviewmessage,"[HIDE]"); $i > 0; $i--) { 
                ...
                $reviewmessage = str_replace($lock_mess, "<font color='red'><b>[VERSTECKTER TEXT]</b></font><br>Du musst auf dieses Thema antworten, um den versteckten Text zu sehen!<br>",  $reviewmessage);
              } 
            } 
            // ##### [HIDE] [/HIDE] HACK End #####
          }
