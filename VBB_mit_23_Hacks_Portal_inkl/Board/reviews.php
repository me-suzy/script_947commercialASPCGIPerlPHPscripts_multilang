...
function alpha() { 
    ...
    echo " ]</center><br><br>\n\n\n";
    echo "<center>[ <a href=\"reviews.php?op=write_review\">"._WRITEREVIEW."</a> ]</center><br><br>\n\n";
}
...
function write_review() {
    global $admin, $sitename, $user, $cookie, $prefix;
    ...
    if (is_admin($admin)) {
	echo "<font size=\"2\">"._PAGEBREAK."</font><br>";
    }
    echo "
    <i>"._CHECKREVIEW."</i><br><br>
    <b>"._YOURNAME.":</b><br>";
    if (is_user($user)) {
	// Changes for vbPortal
      global $name,$email;
    }
...
function preview_review($date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $hits, $id) {
	...
	if ($date == "")
	    ...
	    echo "<p><font size=\"4\"><i><b>$title</b></i></font><br>";
	    echo "<blockquote><p>";
	    if ($cover != "")
	    ...
	    echo "<input type=\"hidden\" name=\"id\" value=$id>
		  ...
		  <input type=\"submit\" name=\"op\" value=\""._YES."\"> <input type=\"button\" onClick=\"history.go(-1)\" value=\""._NO."\">";
	    if($id != 0)
	    	$word = ""._RMODIFIED."";
	    else
	    	$word = ""._RADDED."";
...
function send_review($date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $hits, $id) {
    global $admin, $EditedMessage, $prefix;
    include ('header.php');
    if (eregi("<!--pagebreak-->", $text)) {
	$text = ereg_replace("<!--pagebreak-->","&lt;!--pagebreak--&gt;;",$text);
    }
	...
	mysql_query("INSERT INTO $prefix"._reviews." VALUES (NULL, '$date', '$title', '$text', '$reviewer', '$email', '$score', '$cover', '$url', '$url_title', '1')");
	echo ""._ISAVAILABLE."";
    } else if ((is_admin($admin)) && ($id != 0)) {
	mysql_query("UPDATE $prefix"._reviews." SET date='$date', title='$title', text='$text', reviewer='$reviewer', email='$email', score='$score', cover='$cover', url='$url', url_title='$url_title', hits='$hits' where id = $id");
	echo ""._ISAVAILABLE."";
    } else {
	mysql_query("INSERT INTO $prefix"._reviews."_add VALUES (NULL, '$date', '$title', '$text', '$reviewer', '$email', '$score', '$url', '$url_title')");
	echo ""._EDITORWILLLOOK."";
    }
    echo "<br><br>[ <a href=\"reviews.php\">"._RBACK."</a> ]<br></center>";
    CloseTable();
    include ("footer.php");
}

function reviews_index() {
    global $bgcolor4, $bgcolor2, $prefix;
    include ('header.php');
    OpenTable();	
    echo "<table border=\"0\" width=\"95%\" CELLPADDING=\"2\" CELLSPACING=\"4\" align=\"center\">
    <tr><td colspan=\"2\"><center><font size=\"4\">"._RWELCOME."</font></center><br><br><br>";
    $result = mysql_query("select title, description from $prefix"._reviews."_main");
    ...
    $result_pop = mysql_query("select id, title, hits from $prefix"._reviews." order by hits DESC limit 10");
    $result_rec = mysql_query("select id, title, date, hits from $prefix"._reviews." order by date DESC limit 10");
    $y = 1;
    for ($x = 0; $x < 10; $x++)	{
    ...
    $result = mysql_query("SELECT * FROM $prefix"._reviews."");
...
function reviews($letter, $field, $order) {
    global $bgcolor4, $sitename, $prefix;
    include ('header.php');
    OpenTable();
	...
	case "reviewer":
	$result = mysql_query("SELECT id, title, hits, reviewer, score FROM $prefix"._reviews." WHERE UPPER(title) LIKE '$letter%' ORDER by reviewer $order");
	break;

	case "score":
	$result = mysql_query("SELECT id, title, hits, reviewer, score FROM $prefix"._reviews." WHERE UPPER(title) LIKE '$letter%' ORDER by score $order");
	break;

	case "hits":
	$result = mysql_query("SELECT id, title, hits, reviewer, score FROM $prefix"._reviews." WHERE UPPER(title) LIKE '$letter%' ORDER by hits $order");
	break;

	default:
	$result = mysql_query("SELECT id, title, hits, reviewer, score FROM $prefix"._reviews." WHERE UPPER(title) LIKE '$letter%' ORDER by title $order");
	break;
	...
	echo "<TABLE BORDER=\"0\" width=\"100%\" CELLPADDING=\"2\" CELLSPACING=\"4\">
		...
		<P ALIGN=\"LEFT\"><a href=\"reviews.php?op=$letter&amp;field=title&amp;order=ASC\"><img src=\"images/download/up.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTASC."\"></a><B> "._PRODUCTTITLE." </B><a href=\"reviews.php?op=$letter&amp;field=title&amp;order=DESC\"><img src=\"images/download/down.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTDESC."\"></a>
		</td>
		<td width=\"18%\" bgcolor=\"$bgcolor4\">
		<P ALIGN=\"CENTER\"><a href=\"reviews.php?op=$letter&amp;field=reviewer&amp;order=ASC\"><img src=\"images/download/up.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTASC."\"></a><B> "._REVIEWER." </B><a href=\"reviews.php?op=$letter&amp;field=reviewer&amp;order=desc\"><img src=\"images/download/down.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTDESC."\"></a>
		</td>
		<td width=\"18%\" bgcolor=\"$bgcolor4\">
		<P ALIGN=\"CENTER\"><a href=\"reviews.php?op=$letter&amp;field=score&amp;order=ASC\"><img src=\"images/download/up.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTASC."\"></a><B> "._SCORE." </B><a href=\"reviews.php?op=$letter&amp;field=score&amp;order=DESC\"><img src=\"images/download/down.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTDESC."\"></a>
		</td>
		<td width=\"14%\" bgcolor=\"$bgcolor4\">
		<P ALIGN=\"CENTER\"><a href=\"reviews.php?op=$letter&amp;field=hits&amp;order=ASC\"><img src=\"images/download/up.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTASC."\"></a><B> "._HITS." </B><a href=\"reviews.php?op=$letter&amp;field=hits&amp;order=DESC\"><img src=\"images/download/down.gif\" border=\"0\" width=\"15\" height=\"9\" Alt=\""._SORTDESC."\"></a>
		</td>
		</tr>";
    ...
    echo "[ <a href=\"reviews.php\">"._RETURN2MAIN."</a> ]";
    /* memory flush */
    mysql_free_result($result);
    CloseTable();
    include ("footer.php");
...
function postcomment($id, $title) {
    ...
    OpenTable();
    echo "<center><font size=3><b>"._REVIEWCOMMENT." $title</b><br><br></font></center>
...
function savecomment($xanonpost, $uname, $id, $score, $comments) {
    global $anonymous, $user, $cookie, $prefix;
    if ($xanonpost) {
	$uname = $anonymous;
    }
    $comments = stripslashes(FixQuotes(check_html($comments)));
    mysql_query("insert into $prefix"._reviews."_comments values (NULL, '$id', '$uname', now(), '$comments', '$score')");
    Header("Location: reviews.php?op=showcontent&id=$id");
}

function r_comments($id, $title) {
    global $admin, $prefix;
    $result = mysql_query("select cid, userid, date, comments, score from $prefix"._reviews."_comments where rid='$id' ORDER BY date DESC");
	...
	if ($uname == "Anonymous") {
	    ...
	    echo ""._POSTEDBY." <a href=\"user.php?op=userinfo&amp;uname=$uname\">$uname</a> "._ON." $date<br>";
	}
	echo ""._MYSCORE." ";
	display_score($score);
	if (is_admin($admin)) {
	    echo "<br><b>"._ADMIN."</b> [ <a href=\"reviews.php?op=del_comment&amp;cid=$cid&amp;id=$id\">"._DELETE."</a> ]</font><hr noshade size=1><br><br>";
	} else {
...
function showcontent($id, $page) {
	global $admin, $uimages, $prefix;
	include ('header.php');
	
	OpenTable();
    if (($page == 1) OR ($page == "")) {
	mysql_query("UPDATE $prefix"._reviews." SET hits=hits+1 WHERE id=$id");
    }	
	$result = mysql_query("SELECT * FROM $prefix"._reviews." WHERE id=$id");
	...
	if (is_admin($admin))
		echo "<b>"._ADMIN."</b> [ <a href=\"reviews.php?op=mod_review&amp;id=$id\">"._EDIT."</a> | <a href=reviews.php?op=del_review&amp;id_del=$id>"._DELETE."</a> ]<br>";
	echo "<b>"._ADDED."</b> $fdate<br>";
	if ($reviewer != "")
	echo "<b>"._REVIEWER."</b> <a href=mailto:$email>$reviewer</a><br>";
	if ($score != "")
    ...
    if($page >= $pageno)
 {
	...
	$next_page .= "<a href=\"reviews.php?op=showcontent&amp;id=$id&amp;page=$next_pagenumber\">"._NEXT." ($next_pagenumber/$pageno)</a> <a href=\"reviews.php?op=showcontent&amp;id=$id&amp;page=$next_pagenumber\"><img src=\"images/download/right.gif\" border=\"0\" alt=\""._NEXT."\"></a>";
    }
    ...
    if($page <= 1)
 {
	...
	$previous_page = "<a href=\"reviews.php?op=showcontent&amp;id=$id&amp;page=$previous_pagenumber\"><img src=\"images/download/left.gif\" border=\"0\" alt=\""._PREVIOUS."\"></a> <a href=\"reviews.php?op=showcontent&amp;id=$id&amp;page=$previous_pagenumber\">"._PREVIOUS." ($previous_pagenumber/$pageno)</a>";
    }
    echo "</td></tr>"
	."<tr><td align=\"center\">"
	."$previous_page &nbsp;&nbsp; $next_page<br><br>"
	."[ <a href=\"reviews.php\">"._RBACK."</a> | "
	."<a href=\"reviews.php?op=postcomment&amp;id=$id&amp;title=$title\">"._REPLYMAIN."</a> ]";
    CloseTable();
    if (($page == 1) OR ($page == "")) {
	echo "<br>";
	r_comments($id, $title);
...
function mod_review($id) {
	...
	else if (($id != 0) && (is_admin($admin)))
	{
		...
		echo "<input type=hidden name=op value=preview_review><input type=submit value=\""._PREMODS."\">&nbsp;&nbsp;<input type=button onClick=history.go(-1) value="._CANCEL."></form>";
		// memory flush
		mysql_free_result($result);
	}
	CloseTable();
...
function del_review($id_del) {
	global $admin, $prefix;
	if (is_admin($admin)) {
		mysql_query("delete from $prefix"._reviews." where id = $id_del");
		mysql_query("delete from $prefix"._reviews."_comments where rid='$id_del'");
...
function del_comment($cid, $id) {
	global $admin, $prefix;
	if (is_admin($admin)) {
	    mysql_query("delete from $prefix"._reviews."_comments where cid='$cid'");
	    Header("Location: reviews.php?op=showcontent&id=$id");
	} else {
