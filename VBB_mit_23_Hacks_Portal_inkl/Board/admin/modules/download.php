...
function downloads() {
    ...
    OpenTable();
    echo "<center><a href=\"download.php\"><img src=\"images/download/down-logo.gif\" border=\"0\" alt=\"\"></a><br><br>";
...
/* Temporarily 'homeless' downloads functions (to be revised in admin.php breakup) */
    ...
    $result = mysql_query("select * from $prefix"._downloads_modrequest." where brokendownload=1");
    $totalbrokendownloads = mysql_num_rows($result);
    $result2 = mysql_query("select * from $prefix"._downloads_modrequest." where brokendownload=0");
    $totalmodrequests = mysql_num_rows($result2);

/* List Downloads waiting for validation */

    $result = mysql_query("select lid, cid, sid, title, url, description, name, email, submitter, filesize, version, homepage from $prefix"._downloads_newdownload." order by lid");
    $numrows = mysql_num_rows($result);
    if ($numrows>0) {
	OpenTable();
	echo "<center><font class=\"option\"><b>"._DOWNLOADSWAITINGVAL."</b></font></center><br><br>";
	while(list($lid, $cid, $sid, $title, $url, $description, $name, $email, $submitter, $filesize, $version, $homepage) = mysql_fetch_row($result)) {
	    if ($submitter == "") {
		$submitter = _NONE;
	    }
	    ...
	    echo "<input type=\"hidden\" name=\"submitter\" value=\"$submitter\">";
	    echo "</select><input type=\"hidden\" name=\"op\" value=\"DownloadsAddDownload\"><br><br><input type=\"submit\" value="._ADD."> [ <a href=\"admin.php?op=DownloadsDelNew&amp;lid=$lid\">"._DELETE."</a> ]</form><br><hr noshade><br>";
	}
	CloseTable();
	echo "<br>";
    } else {
...
/* Add a New Main Category */
	...
	."<font class=\"option\"><b>"._ADDMAINCATEGORY."</b><br><br>"
...
// Add a New Download to Database
	...
	OpenTable();
	echo "<form method=\"post\" action=\"admin.php\">"
	    ."<font class=\"option\"><b>"._ADDNEWDOWNLOAD."</b><br><br>"
...
// Modify Downloads
    ...
    OpenTable();
    echo "<form method=\"post\" action=\"admin.php\">"
	."<font class=\"option\"><b>"._MODDOWNLOAD."</b><br><br>"
...
function DownloadsModDownload($lid) {
    ...
    GraphicAdmin($hlpfile);
    global $anonymous;
    $result = mysql_query("select cid, sid, title, url, description, name, email, hits, filesize, version, homepage from $prefix"._downloads_downloads." where lid=$lid");
    OpenTable();
    echo "<center><font class=\"title\"><b>"._WEBDOWNLOADSADMIN."</b></font></center>";
    CloseTable();
    echo "<br>";
    OpenTable();
    echo "<center><font class=\"option\"><b>"._MODDOWNLOAD."</b></font></center><br><br>";
    while(list($cid, $sid, $title, $url, $description, $name, $email, $hits, $filesize, $version, $homepage) = mysql_fetch_row($result)) {
    ...
    /* Modify or Add Editorial */
        ...
        $resulted2 = mysql_query("select adminid, editorialtimestamp, editorialtext, editorialtitle from $prefix"._downloads_editorials." where downloadid=$lid");
    ...
    OpenTable();
    /* Show Comments */
    $result5=mysql_query("SELECT ratingdbid, ratinguser, ratingcomments, ratingtimestamp FROM $prefix"._downloads_votedata." WHERE ratinglid = $lid AND ratingcomments != '' ORDER BY ratingtimestamp DESC");
    ...
    $result5=mysql_query("SELECT ratingdbid, ratinguser, rating, ratinghostname, ratingtimestamp FROM $prefix"._downloads_votedata." WHERE ratinglid = $lid AND ratinguser != 'outside' AND ratinguser != '$anonymous' ORDER BY ratingtimestamp DESC");
    ...
    $result5=mysql_query("SELECT ratingdbid, rating, ratinghostname, ratingtimestamp FROM $prefix"._downloads_votedata." WHERE ratinglid = $lid AND ratinguser = '$anonymous' ORDER BY ratingtimestamp DESC");
    $totalvotes = Mysql_num_rows($result5);  
    ...
    $result5=mysql_query("SELECT ratingdbid, rating, ratinghostname, ratingtimestamp FROM $prefix"._downloads_votedata." WHERE ratinglid = $lid AND ratinguser = 'outside' ORDER BY ratingtimestamp DESC");
    $totalvotes = Mysql_num_rows($result5);  
...
function DownloadsDelComment($lid, $rid) {
    global $prefix;
    mysql_query("UPDATE $prefix"._downloads_votedata." SET ratingcomments='' WHERE ratingdbid = $rid");
    mysql_query("UPDATE $prefix"._downloads_downloads." SET totalcomments = (totalcomments - 1) WHERE lid = $lid");
...
function DownloadsDelVote($lid, $rid) {
    global $prefix;
    mysql_query("delete from $prefix"._downloads_votedata." where ratingdbid=$rid");
    ...
    mysql_query("UPDATE $prefix"._downloads_downloads." SET downloadratingsummary=$finalrating,totalvotes=$totalvotesDB,totalcomments=$truecomments WHERE lid = $lid");
    Header("Location: admin.php?op=DownloadsModDownload&lid=$lid");
}
...
function DownloadsListBrokenDownloads() {
    ...
    echo "<br>";
    OpenTable();
    $result = mysql_query("select requestid, lid, modifysubmitter from $prefix"._downloads_modrequest." where brokendownload=1 order by requestid");
    $totalbrokendownloads = mysql_num_rows($result);
    echo "<center><font class=\"option\"><b>"._DUSERREPBROKEN." ($totalbrokendownloads)</b></font></center><br><br><center>"
	.""._DIGNOREINFO."<br>"
	.""._DDELETEINFO."</center><br><br><br>"
	."<table align=\"center\" width=\"450\">";
    if ($totalbrokendownloads==0) {
	echo "<center><font class=\"option\">"._DNOREPORTEDBROKEN."</font></center><br><br><br>";
    } else {
	    ...
	    $result2 = mysql_query("select title, url, submitter from $prefix"._downloads_downloads." where lid=$lid");
	    if ($modifysubmitter != '$anonymous') {
		$result3 = mysql_query("select email from user where username='$modifysubmitter'");
		list($email)=mysql_fetch_row($result3);
	    }
    	    list($title, $url, $owner)=mysql_fetch_row($result2);
    	    $result4 = mysql_query("select email from user where username='$owner'");
...
function DownloadsDelBrokenDownloads($lid) {
    global $prefix;
    mysql_query("delete from $prefix"._downloads_modrequest." where lid=$lid");
    mysql_query("delete from $prefix"._downloads_downloads." where lid=$lid");
    Header("Location: admin.php?op=DownloadsListBrokenDownloads");
}

function DownloadsIgnoreBrokenDownloads($lid) {
    global $prefix;
    mysql_query("delete from $prefix"._downloads_modrequest." where lid=$lid and brokendownload=1");
    Header("Location: admin.php?op=DownloadsListBrokenDownloads");
}
...
function DownloadsListModRequests() {
    ...
    echo "<br>";
    OpenTable();
    $result = mysql_query("select requestid, lid, cid, sid, title, url, description, modifysubmitter, name, email, filesize, version, homepage from $prefix"._downloads_modrequest." where brokendownload=0 order by requestid");
    $totalmodrequests = mysql_num_rows($result);	
    echo "<center><font class=\"option\"><b>"._DUSERMODREQUEST." ($totalmodrequests)</b></font></center><br><br><br>";
    echo "<table width=\"95%\"><tr><td>";
    while(list($requestid, $lid, $cid, $sid, $title, $url, $description, $modifysubmitter, $name, $email, $filesize, $version, $homepage)=mysql_fetch_row($result)) {
	$result2 = mysql_query("select cid, sid, title, url, description, name, email, submitter, filesize, version, homepage from $prefix"._downloads_downloads." where lid=$lid");
	list($origcid, $origsid, $origtitle, $origurl, $origdescription, $origname, $origemail, $owner, $origfilesize, $origversion, $orighomepage)=mysql_fetch_row($result2);
	$result3 = mysql_query("select title from $prefix"._downloads_categories." where cid=$cid");
	$result4 = mysql_query("select title from $prefix"._downloads_subcategories." where cid=$cid and sid=$sid");
	$result5 = mysql_query("select title from $prefix"._downloads_categories." where cid=$origcid");
	$result6 = mysql_query("select title from $prefix"._downloads_subcategories." where cid=$origcid and sid=$origsid");
	$result7 = mysql_query("select email from user where username='$modifysubmitter'");
	$result8 = mysql_query("select email from user where username='$owner'");
...
function DownloadsChangeModRequests($requestid) {
    global $prefix;
    $result = mysql_query("select requestid, lid, cid, sid, title, url, description, name, email, filesize, version, homepage from $prefix"._downloads_modrequest." where requestid=$requestid");
    while(list($requestid, $lid, $cid, $sid, $title, $url, $description, $name, $email, $filesize, $version, $homepage)=mysql_fetch_row($result)) {
	$title = stripslashes($title);
    	$description = stripslashes($description);
    	mysql_query("UPDATE $prefix"._downloads_downloads." SET cid=$cid, sid=$sid, title='$title', url='$url', description='$description', name='$name', email='$email', filesize='$filesize', version='$version', homepage='$homepage' WHERE lid = $lid");
	mysql_query("delete from $prefix"._downloads_modrequest." where requestid=$requestid");
    }
...
function DownloadsCleanVotes() {
    global $prefix;
    $totalvoteresult = mysql_query("select distinct ratinglid FROM $prefix"._downloads_votedata."");	
...
function DownloadsModDownloadS($lid, $title, $url, $description, $name, $email, $hits, $cat, $filesize, $version, $homepage) {
    ...
    mysql_query("update $prefix"._downloads_downloads." set cid='$cat[0]', sid='$cat[1]', title='$title', url='$url', description='$description', name='$name', email='$email', hits='$hits', filesize='$filesize', version='$version', homepage='$homepage' where lid=$lid");
    Header("Location: admin.php?op=downloads");
}
...
function DownloadsModCat($cat) {
    ...
    if ($cat[1]==0) {
	$result=mysql_query("select title, cdescription from $prefix"._downloads_categories." where cid=$cat[0]");
	...
	$result=mysql_query("select title from $prefix"._downloads_categories." where cid=$cat[0]");
	list($ctitle) = mysql_fetch_row($result);
	$result2=mysql_query("select title from $prefix"._downloads_subcategories." where sid=$cat[1]");
...
function DownloadsModCatS($cid, $sid, $sub, $title, $cdescription) {
    global $prefix;
    if ($sub==0) {
	mysql_query("update $prefix"._downloads_categories." set title='$title', cdescription='$cdescription' where cid=$cid");
    } else {
	mysql_query("update $prefix"._downloads_subcategories." set title='$title' where sid=$sid");
    }
    Header("Location: admin.php?op=downloads");
}
...
function DownloadsDelCat($cid, $sid, $sub, $ok=0) {
    ...
    if($ok==1) {
	if ($sub>0) {
    	    mysql_query("delete from $prefix"._downloads_subcategories." where sid=$sid");
	    mysql_query("delete from $prefix"._downloads_downloads." where sid=$sid");
	} else {
	    mysql_query("delete from $prefix"._downloads_categories." where cid=$cid");
	    mysql_query("delete from $prefix"._downloads_subcategories." where cid=$cid");
	    mysql_query("delete from $prefix"._downloads_downloads." where cid=$cid AND sid=0");
	}
	Header("Location: admin.php?op=downloads");    
    } else {
	include("header.php");
	...
	echo "[ <a href=\"admin.php?op=DownloadsDelCat&amp;cid=$cid&amp;sid=$sid&amp;sub=$sub&amp;ok=1\">"._YES."</a> | <a href=\"admin.php?op=downloads\">"._NO."</a> ]<br><br>";
	CloseTable();
	include("footer.php");	
}
...
function DownloadsDelNew($lid) {
    global $prefix;
    mysql_query("delete from $prefix"._downloads_newdownload." where lid=$lid");
    Header("Location: admin.php?op=downloads");
}

function DownloadsAddCat($title, $cdescription) {
    global $prefix;
    $result = mysql_query("select cid from $prefix"._downloads_categories." where title='$title'");
    $numrows = mysql_num_rows($result);
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin($hlpfile);
	...
	mysql_query("insert into $prefix"._downloads_categories." values (NULL, '$title', '$cdescription')");
	Header("Location: admin.php?op=downloads");
    }
}
...
function DownloadsAddSubCat($cid, $title) {
    global $prefix;
    $result = mysql_query("select cid from $prefix"._downloads_subcategories." where title='$title' AND cid='$cid'");
    $numrows = mysql_num_rows($result);
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin($hlpfile);
	...
	mysql_query("insert into $prefix"._downloads_subcategories." values (NULL, '$cid', '$title')");
	Header("Location: admin.php?op=downloads");
    }
}
...
function DownloadsAddEditorial($downloadid, $editorialtitle, $editorialtext) {
    ...
    mysql_query("insert into $prefix"._downloads_editorials." values ($downloadid, '$aid', now(), '$editorialtext', '$editorialtitle')");
    include("header.php");
    GraphicAdmin($hlpfile);
    OpenTable();
    echo "<center><br>"
...
function DownloadsModEditorial($downloadid, $editorialtitle, $editorialtext) {
    global $prefix;
    $editorialtext = stripslashes(FixQuotes($editorialtext));
    mysql_query("update $prefix"._downloads_editorials." set editorialtext='$editorialtext', editorialtitle='$editorialtitle' where downloadid=$downloadid");
    include("header.php");
    GraphicAdmin($hlpfile);
    OpenTable();
    echo "<br><center>"
...
function DownloadsDelEditorial($downloadid) {
    global $prefix;
    mysql_query("delete from $prefix"._downloads_editorials." where downloadid=$downloadid");
    include("header.php");
    GraphicAdmin($hlpfile);
    OpenTable();
    echo "<br><center>"
...
function DownloadsDownloadCheck() {
    ...
    echo "<br>";
    OpenTable();
    echo "<center><font class=\"option\"><b>"._DOWNLOADVALIDATION."</b></font></center><br>"
    ...
    $result = mysql_query("select sid, cid, title from $prefix"._downloads_subcategories." order by title");    
...
function DownloadsValidate($cid, $sid, $ttitle) {
	...
	$result = mysql_query("select lid, title, url, name, email, submitter from $prefix"._downloads_downloads." order by title");    
    }   	
	...
	$result = mysql_query("select lid, title, url, name, email, submitter from $prefix"._downloads_downloads." where cid=$cid order by title");    
    }
    /* Check Only Subcategory */
    if ($cid==0 && $sid!=0) {
   	echo "<tr><td colspan=\"3\"><center><b>"._VALIDATINGSUBCAT.": $transfertitle</b><br>"._BEPATIENT."</center><br><br></td></tr>";
   	$result = mysql_query("select lid, title, url, name, email, submitter from $prefix"._downloads_downloads." where sid=$sid order by title");
    }
...
// Check if Description exist
    ...
    $name = stripslashes(FixQuotes($name));
    $email = stripslashes(FixQuotes($email));
    mysql_query("insert into $prefix"._downloads_downloads." values (NULL, '$cat[0]', '$cat[1]', '$title', '$url', '$description', now(), '$name', '$email', '$hits','$submitter',0,0,0, '$filesize', '$version', '$homepage')");
