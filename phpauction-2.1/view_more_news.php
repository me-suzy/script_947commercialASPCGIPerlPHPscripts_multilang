 ...
 */
	...
	if(!$offset) $offset = 0;
		...
		$query = "select count(id) as auctions from PHPAUCTION_auctions WHERE closed='0' ";
	...
	$result = mysql_query ( "SELECT * FROM PHPAUCTION_auctions WHERE closed='0' ORDER BY starts DESC LIMIT $offset,$limit" );
...

		{
			...
			$tplv .= "<TR ALIGN=CENTER VALIGN=MIDDLE BGCOLOR=\"$bgColor\">";
				...
				if(strlen($seconds_difference) == 1){
					...
					$tplv .= "<TD>$std_font $days_difference $MSG_097
	...
	include "templates/template_view_news_php.html";
