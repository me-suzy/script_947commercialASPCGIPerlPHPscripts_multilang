...
//END GOBLINS USER LEVELS
	...
	if ($HIDE_shown == 0) { 
		for ($i = lock_count($post[message],"[HIDE]"); $i > 0; $i--) { 
			...
			$post[message] = str_replace( $lock_mess, "<font color='red'><b>[VERSTECKTER TEXT]</b></font><br>Du musst auf dieses Thema antworten, um den versteckten Text zu sehen!<br>", $post[message]);
		} 
	} 
        // ##### [HIDE] [/HIDE] HACK End #####
...
function deletethread($threadid,$countposts=1) {
  ...
  if ($threadinfo=getthreadinfo($threadid)) {
    ...
    $DB_site->query("DELETE FROM bedankomat WHERE threadid='$threadid'");
  }
}
...
function getcodebuttons () {
	...
	eval ("\$vbcode_colorbits = \"".gettemplate("vbcode_colorbits")."\";");
        eval ("\$vbcode_alignbits = \"".gettemplate("vbcode_alignbits")."\";");
        eval ("\$vbcode_cursors = \"".gettemplate("vbcode_cursors")."\";");
