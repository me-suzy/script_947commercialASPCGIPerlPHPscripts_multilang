...
function BlocksAdmin() {
	...
	OpenTable();
	print '<center><font size="4"><b>Zentrale Blöcke</b></font>';
	if($show_inactive) {
		print ' <font size="1">[ <a href="admin.php?op=CenterBlocksAdmin">deaktivieren</a> ]</font>';
	}
	else {
		print ' <font size="1">[ <a href="admin.php?op=CenterBlocksAdmin&amp;show_inactive=1">aktivieren</a> ]</font>';
	}
	print '</center><br>'
	...
	$high[l] = mysql_fetch_array(mysql_query("SELECT weight FROM nuke_centerblocks WHERE position='l' $filter_weight ORDER BY weight DESC"));
	$high[r] = mysql_fetch_array(mysql_query("SELECT weight FROM nuke_centerblocks WHERE position='r' $filter_weight ORDER BY weight DESC"));
	$result = mysql_query("SELECT bid, bkey, title, url, position, weight, active FROM nuke_centerblocks $filter_inactive ORDER BY position, weight");
	...
	while($row = mysql_fetch_array($result)) {
		...
		$type = $centerblocks_modules[$row[bkey]][text_type];
		//
		// weight
		//
		$move_up = '';
		...
		if($row[position] == $prev_pos) {
			$move_up = "<a href=\"admin.php?op=CenterBlocksOrder&amp;bid=$row[bid]&amp;position=$row[position]&amp;new_weight=" . ($row[weight] - 1.5) . "\"><img src=\"images/common/up_thin.gif\" border=\"0\" alt=\"" . _BLOCKUP . "\"></a>";
		}
		if($row[weight] != $high[$row[position]][weight]) {
			$move_down = "<a href=\"admin.php?op=CenterBlocksOrder&amp;bid=$row[bid]&amp;position=$row[position]&amp;new_weight=" . ($row[weight] + 1.5) . "\"><img src=\"images/common/down_thin.gif\" border=\"0\" alt=\"" . _BLOCKDOWN . "\"></a>";
		}
		...
		// functions
		//
		print "<td align=\"left\"><a href=\"admin.php?op=CenterBlocksEdit&amp;bid=$row[bid]\"><img src=\"images/common/edit.gif\" border=\"0\" alt=\"Editiere Block\"></a> <a href=\"admin.php?op=CenterBlocksChangeStatus&amp;bid=$row[bid]\"><img src=\"$change_img\" border=\"0\" alt=\"$change\"></a>";
		if($centerblocks_modules[$row[bkey]][allow_delete]) {
			print " <a href=\"admin.php?op=CenterBlocksDelete&amp;bid=$row[bid]\"><img src=\"images/common/delete.gif\" border=\"0\" alt=\"Lösche Block\"></a>";
		}
		print '</td></tr>';
	...
	print '<center><font size="3"><b>' . _ADDNEWBLOCK . '</b></font></center><br>'
		...
		.'<select name="bkey">'
	;
	foreach($centerblocks_modules as $k=>$v) {
		if($v[allow_create]) {
	...
	print '</select>&nbsp;&nbsp;'
		...
		.'<input type="hidden" name="op" value="CenterBlocksAdd"></form>'
		.'</font>'
	;
	CloseTable();
	include 'footer.php';
...
function BlocksAdd() {
	...
	;
	$vars = $HTTP_POST_VARS;
	$high = mysql_fetch_array(mysql_query("SELECT weight FROM nuke_centerblocks WHERE position='l' ORDER BY weight DESC"));
	...
	if($centerblocks_modules[$vars[bkey]][func_add]) {
		$vars = $centerblocks_modules[$vars[bkey]][func_add]($vars);
	}
	mysql_query("INSERT INTO nuke_centerblocks (bid, bkey, title, content, url, position, weight, active, templates, refresh, last_update) VALUES (NULL, '$vars[bkey]', '$vars[title]', '$vars[content]', '$vars[url]', 'l', '$vars[weight]', '$vars[active]', '$vars[templates]','$vars[refresh]', 0)");
	$bid = mysql_insert_id();
	header("Location: admin.php?op=CenterBlocksEdit&bid=$bid");
}
...
function BlocksEdit($bid) {
	...
	// $text_pos[l] = _LEFT;
	// $text_pos[r] = _RIGHT;
	$result = mysql_query("SELECT bid, bkey, title, content, url, position, weight, active, templates, refresh, last_update from nuke_centerblocks WHERE bid='$bid'");
	...
	OpenTable();
	print '<center><font size="3"><b>' . _BLOCK . ": $row[title] (" . $centerblocks_modules[$row[bkey]][text_content] . ")</b></font></center><br><br>"
		.'<form action="admin.php" method="post">'
		.'<table border="0" width="100%">'
		.'<tr><td>' . _TITLE . ':</td><td><input type="text" name="title" size="30" maxlength="60" value="' . htmlentities($row[title]) . '">&nbsp;&nbsp;' . $type . '</td></tr>'
	;
	if($centerblocks_modules[$row[bkey]][form_url]) {
		if($centerblocks_modules[$row[bkey]][url_text]) {
			$url_text = $centerblocks_modules[$row[bkey]][url_text];
		}
		else {
			$url_text = _AB_LABEL_URL;
		}
		if($centerblocks_modules[$row[bkey]][url_size]) {
			$url_size = $centerblocks_modules[$row[bkey]][url_size];
		}
		else {
			$url_size = 30;
		}
		if($centerblocks_modules[$row[bkey]][url_maxlength]) {
			$url_maxlength = $centerblocks_modules[$row[bkey]][url_maxlength];
		}
		else {
			$url_maxlength = 255;
		}
		switch($centerblocks_modules[$row[bkey]][url_type]) {
			default:
			case 'text':
			...
			case 'checkbox':
           	...
           	case 'select':
				...
				foreach($centerblocks_modules[$row[bkey]][url_values] as $k=>$v) {
					$sel = $select_row[$k];
	...
	if($centerblocks_modules[$row[bkey]][form_content]) {
	...
	$result = mysql_query("SELECT title, weight, position FROM nuke_centerblocks WHERE bid!=$bid ORDER BY position, weight");
    ...
    print '</td></tr>';
    if($centerblocks_modules[$row[bkey]][has_templates]) {
		print '<tr><td>' . _TEMPLATES . '</td><td><input type="checkbox" name="templates" value="1"' . $templates[1] . '></td></tr>';
	}
	if($centerblocks_modules[$row[bkey]][form_refresh]) {
	...
	print '</table>'
		...
		.'<input type="hidden" name="op" value="CenterBlocksEditSave">'
...
function BlocksEditSave() {
	...
	;
	$vars = $HTTP_POST_VARS;
	$result = mysql_query("SELECT weight FROM nuke_centerblocks WHERE bid='$vars[bid]'");
	$row = mysql_fetch_array($result);
	$vars[title] = stripslashes(FixQuotes($vars[title]));
	$vars[content] = stripslashes(FixQuotes($vars[content]));
	// let the module do any block-specific changes before updating
	if($centerblocks_modules[$vars[bkey]][func_update]) {
		$vars = $centerblocks_modules[$vars[bkey]][func_update]($vars);
	}
	mysql_query("UPDATE nuke_centerblocks SET title='$vars[title]', content='$vars[content]', url='$vars[url]', position='l', weight='$vars[weight]', active='$vars[active]', templates='$vars[templates]', refresh='$vars[refresh]', last_update=0 WHERE bid='$vars[bid]'");
	if($vars[weight] == $row[weight]) {
		header('Location: admin.php?op=CenterBlocksAdmin');
	}
	else {
		BlocksOrder($vars);
	}
...
function ChangeStatus($bid, $ok=0) {
	global
		...
		$nuketable
	;
	$result = mysql_query("SELECT active from nuke_centerblocks WHERE bid='$bid'");
	$row = mysql_fetch_array($result);
	if($ok || $row[active]) {
		if($row[active] == 0) {
			$active = 1;
		...
		mysql_query("UPDATE nuke_centerblocks SET active='$active' WHERE bid='$bid'");
		header('Location: admin.php?op=CenterBlocksAdmin');
		print '<html><body></body></html>';
		exit;
	}
	$result = mysql_query("SELECT * FROM nuke_centerblocks WHERE bid='$bid'");
	$row = mysql_fetch_array($result);
	include 'header.php';
	GraphicAdmin($hlpfile);
	print '<br>';
	...
	print '<br>';
	OpenTable();
	if($centerblocks_modules[$row[bkey]][show_preview]) {
	...
	print '<br>' . _WANT2ACTIVATE . '<br><br>'
		.'[ <a href="admin.php?op=CenterBlocksAdmin">' . _NO . '</a> | <a href="admin.php?op=CenterBlocksChangeStatus&amp;bid=' . $bid . '&amp;ok=1">' . _YES . '</a> ]'
		.'</center>'
	;
	CloseTable();
	include 'footer.php';
...
function BlocksDelete($bid, $ok=0) {
	...
	if($ok) {
		mysql_query("DELETE from nuke_centerblocks WHERE bid='$bid'");
		header('Location: admin.php?op=CenterBlocksAdmin');
		print '<html><body></body></html>';
		exit;
	}
	...
	print '<br>[ <a href="admin.php?op=CenterBlocksAdmin">' . _NO . '</a> | <a href="admin.php?op=CenterBlocksDelete&amp;bid=' . $bid . '&amp;ok=1">' . _YES . '</a> ]</center>';
	CloseTable();
	include 'footer.php';
}
...
function BlocksOrder($vars) {
	...
	;
	if($vars[new_weight]) {
		mysql_query("UPDATE nuke_centerblocks SET weight='$vars[new_weight]' WHERE bid=$vars[bid]");
	}
	$result = mysql_query("SELECT bid FROM nuke_centerblocks WHERE position='$vars[position]' ORDER BY weight");
	$c = 0;
	while($row = mysql_fetch_array($result)) {
		$c++;
		mysql_query("UPDATE nuke_centerblocks SET weight='$c' WHERE bid=$row[bid]");
	}
	header('Location: admin.php?op=CenterBlocksAdmin');
	print '<html><body></body></html>';
	exit;
