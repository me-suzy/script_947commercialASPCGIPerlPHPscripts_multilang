...
$forumslasttitle1=$DB_site->query("SELECT * FROM lasttitle WHERE field=1");
 while ($forumlasttitle1=$DB_site->fetch_array($forumslasttitle1)) {
  $lasttitle1=$forumlasttitle1[lasttitlelength]; 
	}
if ($forum['showlasttitle']==1) {
	$showlastpostingtitle = $forum['lasttitle'];

if (strlen($showlastpostingtitle) > $lasttitle1) {
           $showlastpostingtitle = substr($showlastpostingtitle,0,$lasttitle1);
...
if ($pagenumber == 1) {
	...
	$stickies = $DB_site->query_first("SELECT COUNT(*) AS stickycount
							FROM thread
							WHERE forumid = $foruminfo[forumid]
								AND visible=1
								AND sticky=1
	...
	$stickycount = $stickies['stickycount'];
}
*/
...
$getthreadids=$DB_site->query("
	...
	".iif($sortfield=="voteavg",$votequery,"")."
		thread.threadid
		".iif($bedankomat_forum[$forumid],', COUNT(bedankomat.threadid) AS thx', '')."
	FROM thread
	".iif($bedankomat_forum[$forumid],'LEFT JOIN bedankomat USING (threadid)', '')."
	WHERE thread.forumid = $foruminfo[forumid]
		AND thread.sticky=0
		AND thread.visible=1
	$datecut
	$limitothers
	".iif($bedankomat_forum[$forumid],'GROUP BY thread.threadid', '')."
...
while ($thread=$DB_site->fetch_array($getthreadids)) {
  ...
  $bedankomate[$thread['threadid']] = $thread['thx'];
}
if ($stickyids != '') {
	$threadids .= $stickyids;
}
...
if ($totalthreads>0 || $stickyids != '') {
  ...
  while ($thread=$DB_site->fetch_array($threads)) { // and $counter++<$perpage) {
    ...
    if ($bedankomate[$thread['threadid']]>0 && $bedankomat_forum[$forumid]==1) {
    	$bedankomat="<br><smallfont>(".$bedankomate[$thread['threadid']]." &times; Danke)</smallfont>";
    	} else {
    		unset($bedankomat);
    	}
    	// Folgendes schöner machen!!!
    	$thread[title].=$bedankomat;
