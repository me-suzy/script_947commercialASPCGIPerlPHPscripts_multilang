...
getvbpvars();
$index = 0;
global  $thememenu,$breadcrumb;
$Pmenu="P_thememenu_downloads";
$breadcrumb="Downloads";
function menu($maindownload) {
    global $Pmenu,$breadcrumb,$user_adddownload;
    OpenTable();
...
function viewdownload($cid, $min, $orderby, $show) {
    ...
    $result=mysql_query("select lid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from $prefix"._downloads_downloads." where cid=$cid AND sid=0 order by $orderby limit $min,$linksperpage ");
    ...
    $downloadpagesint = ($totalselecteddownloads / $linksperpage);			
    $downloadpageremainder = ($totalselecteddownloads % $linksperpage);		
    if ($downloadpageremainder != 0) {					 
    	$downloadpages = ceil($downloadpagesint);				
    	if ($totalselecteddownloads < $linksperpage) {
...
function viewsdownload($sid, $min, $orderby, $show) {
	...
	."<br><b>"._RESSORTED.": $orderbyTrans</b></font></center><br><br>";
    $result=mysql_query("select lid, url, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from $prefix"._downloads_downloads." where sid=$sid order by $orderby limit $min,$linksperpage");
...
function search($query, $min, $orderby, $show) {   
    include("config.php");
    global $admin, $linksperpage, $bgcolor2;
    if (!isset($min)) $min=0;
	...
	$counter = 1;
        ...
        while ($counter<=$downloadpages ) {
    	    $cpage = $counter;
            $mintemp = ($linksperpage * $counter) - $downloadsresults;
            if ($counter == $currentpage) {
		echo "<b>$counter</b> ";
	    } else {
