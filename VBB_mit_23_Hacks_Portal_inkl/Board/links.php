...
function menu($mainlink) {
    OpenTable();
    echo "<br><center><a href=\"links.php\"><img src=\"images/links/web.gif\" border=\"0\" alt=\"\"></a><br><br>";
    echo "<form action=\"links.php?op=search&amp;query=$query\" method=\"post\">"
	."<font size=\"2\"><input type=\"text\" size=\"25\" name=\"query\"> <input type=\"submit\" value=\""._SEARCH."\"></font>"
	."</form>";
    echo "<font size=\"2\">[ ";
    if ($mainlink>0) {
	echo "<a href=\"links.php\">"._LINKSMAIN."</a> | ";
    }
    echo "<a href=\"links.php?op=AddLink\">"._ADDLINK."</a>"
	." | <a href=\"links.php?op=NewLinks\">"._NEW."</a>"
	." | <a href=\"links.php?op=MostPopular\">"._POPULAR."</a>"
	." | <a href=\"links.php?op=TopRated\">"._TOPRATED."</a>"
	." | <a href=\"links.php?op=RandomLink\">"._RANDOM."</a> ]"
	."</font></center>";
    CloseTable();
}
...
function linkinfomenu($lid, $ttitle) {
    echo "<br><font size=\"2\">[ "
	."<a href=\"links.php?op=viewlinkcomments&amp;lid=$lid&amp;ttitle=$ttitle\">"._LINKCOMMENTS."</a>"
	." | <a href=\"links.php?op=viewlinkdetails&amp;lid=$lid&amp;ttitle=$ttitle\">"._ADDITIONALDET."</a>"
	." | <a href=\"links.php?op=viewlinkeditorial&amp;lid=$lid&amp;ttitle=$ttitle\">"._EDITORREVIEW."</a>"
	." | <a href=\"links.php?op=modifylinkrequest&amp;lid=$lid\">"._MODIFY."</a>"
	." | <a href=\"links.php?op=brokenlink&amp;lid=$lid\">"._REPORTBROKEN."</a> ]</font>";
}
...
function index() {
    ...
    $result=mysql_query("select cid, title, cdescription from $prefix"._links_categories." order by title");
    $count = 0;
    while(list($cid, $title, $cdescription) = mysql_fetch_row($result)) {
	$cresult = mysql_query("select * from $prefix"._links_links." where cid=$cid");
	...
	$result2 = mysql_query("select sid, title from $prefix"._links_subcategories." where cid=$cid order by title limit 0,3");
	$space = 0;
    ...
    $result=mysql_query("select * from $prefix"._links_subcategories."");
...
function AddLink() {
    global $user, $prefix, $links_anonaddlinklock;
    include("header.php");
    $mainlink = 1;
    menu(1);
    echo "<br>";
    OpenTable();
    echo "<center><font size=\"4\"><b>"._ADDALINK."</b></font></center><br><br>";
    if (is_user($user) || $links_anonaddlinklock != 1) {
    	...
    	echo "</select><br><br>"
    	    ...
    	    .""._YOURNAME.": <input type=\"text\" name=\"name\" size=\"30\" maxlength=\"60\"><br>"
...
// Check if Description exist
    ...
    mysql_query("insert into $prefix"._links_newlink." values (NULL, '$cat[0]', '$cat[1]', '$title', '$url', '$description', '$name', '$email', '$submitter')");
    include("header.php");
    menu(1);
    echo "<br>";
    OpenTable();
...
function NewLinks($newlinkshowdays) {
    ...
    echo "<br>";
    OpenTable();
    echo "<center><font size=\"3\"><b>"._NEWLINKS."</b></font></center><br>";
    $counter = 0;
    $allweeklinks = 0;
    while ($counter <= 7-1){
	...
	$result = mysql_query("select * FROM $prefix"._links_links." WHERE date LIKE '%$newlinkDB%'");
	...
	.""._SHOW.": <a href=\"links.php?op=NewLinks&amp;newlinkshowdays=7\">"._1WEEK."</a> - <a href=\"links.php?op=NewLinks&amp;newlinkshowdays=14\">"._2WEEKS."</a> - <a href=\"links.php?op=NewLinks&amp;newlinkshowdays=30\">"._30DAYS."</a>"
	."</center><br>";
	...
	$counter++;
	$allweeklinks = $allweeklinks + $totallinks;
	echo "<strong><big>&middot;</big></strong> <a href=\"links.php?op=NewLinksDate&amp;selectdate=$newlinkdayRaw\">$newlinkView</a>&nbsp($totallinks)<br>";
    }
    $counter = 0;
    $allmonthlinks = 0;
    echo "</center>";
...
function NewLinksDate($selectdate) {
    ...
    $result = mysql_query("select * FROM $prefix"._links_links." WHERE date LIKE '%$newlinkDB%'");
    $totallinks = mysql_num_rows($result); 
    echo "<font size=\"3\"><b>$dateView - $totallinks "._NEWLINKS."</b></font>"
	."<table width=\"100%\" cellspacing=\"0\" cellpadding=\"10\" border=\"0\"><tr><td><font size=\"2\">";
    $result=mysql_query("select lid, cid, sid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where date LIKE '%$newlinkDB%' order by title ASC");
	...
	echo ""._ADDEDON.": <b>$datetime</b> "._HITS.": $hits";
        ...
        echo "<br><a href=\"links.php?op=ratelink&amp;lid=$lid&amp;ttitle=$transfertitle\">"._RATESITE."</a>";
        echo " | <a href=\"links.php?op=brokenlink&amp;lid=$lid\">"._REPORTBROKEN."</a>";
	if ($totalvotes != 0) {
	    echo " | <a href=\"links.php?op=viewlinkdetails&amp;lid=$lid&amp;ttitle=$transfertitle\">"._DETAILS."</a>";
	}
        if ($totalcomments != 0) {
	    echo " | <a href=\"links.php?op=viewlinkcomments&amp;lid=$lid&amp;ttitle=$transfertitle\">"._SCOMMENTS." ($totalcomments)</a>";
	}
	...
	$result3=mysql_query("select title from $prefix"._links_subcategories." where sid=$sid");
...
function TopRated($ratenum, $ratetype) {			
    ...
    if ($toplinkspercentrigger == 1) {
    	...
    	$result=mysql_query("select * from $prefix"._links_links." where linkratingsummary != 0");
    ...
    if ($toplinkspercentrigger == 1) { 
	echo "<center><font size=\"3\"><b>"._BESTRATED." $toplinkspercent% ("._OF." $totalratedlinks "._TRATEDLINKS.")</b></font></center><br>";
    } else {
	echo "<center><font size=\"3\"><b>"._BESTRATED." $toplinks </b></font></center><br>";
    }
    echo "</td></tr>"
	."<tr><td><center>"._NOTE." $linkvotemin "._TVOTESREQ."<br>"
	.""._SHOWTOP.":  [ <a href=\"links.php?op=TopRated&amp;ratenum=10&amp;ratetype=num\">10</a> - "
	."<a href=\"links.php?op=TopRated&amp;ratenum=25&amp;ratetype=num\">25</a> - "
    	...
    	."<a href=\"links.php?op=TopRated&amp;ratenum=10&amp;ratetype=percent\">10%</a> ]</center><br><br></td></tr>";
    $result=mysql_query("select lid, cid, sid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where linkratingsummary != 0 and totalvotes >= $linkvotemin order by linkratingsummary DESC limit 0,$toplinks");
    echo "<tr><td>";
    while(list($lid, $cid, $sid, $title, $description, $time, $hits, $linkratingsummary, $totalvotes, $totalcomments)=mysql_fetch_row($result)) {
	$linkratingsummary = number_format($linkratingsummary, $mainvotedecimal);
...
function MostPopular($ratenum, $ratetype) {
    ...
    if ($mostpoplinkspercentrigger == 1) {
	echo "<center><font size=\"3\"><b>"._MOSTPOPULAR." $toplinkspercent% ("._OFALL." $totalmostpoplinks "._LINKS.")</b></font></center>";
    } else {
	echo "<center><font size=\"3\"><b>"._MOSTPOPULAR." $mostpoplinks</b></font></center>";
    }
    echo "<tr><td><center>"._SHOWTOP.": [ <a href=\"links.php?op=MostPopular&amp;ratenum=10&amp;ratetype=num\">10</a> - "
	."<a href=\"links.php?op=MostPopular&amp;ratenum=25&amp;ratetype=num\">25</a> - "
    	...
    	."<a href=\"links.php?op=MostPopular&amp;ratenum=10&amp;ratetype=percent\">10%</a> ]</center><br><br></td></tr>";
    $result=mysql_query("select lid, cid, sid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." order by hits DESC limit 0,$mostpoplinks");
    echo "<tr><td>";
    while(list($lid, $cid, $sid, $title, $description, $time, $hits, $linkratingsummary, $totalvotes, $totalcomments)=mysql_fetch_row($result)) {
	$linkratingsummary = number_format($linkratingsummary, $mainvotedecimal);
	$title = stripslashes($title);
	$description = stripslashes($description);
        echo "<font size=\"2\"><a href=\"links.php?op=visit&amp;lid=$lid\" target=\"new\">$title</a>";
	newlinkgraphic($datetime, $time);
	popgraphic($hits);
	echo "<br>";
	echo ""._DESCRIPTION.": $description<br>";
...
function RandomLink() {
    ...
    $result2 = mysql_query("select url from $prefix"._links_links." where lid='$random'");
    list($url) = mysql_fetch_row($result2);
    mysql_query("update $prefix"._links_links." set hits=hits+1 where lid=$random");
    Header("Location: $url");
}

function viewlink($cid, $min, $orderby, $show) {
    global $admin, $linksperpage, $prefix;
    include("header.php");
    if (!isset($min)) $min=0;
    if (!isset($max)) $max=$min+$linksperpage;
    if(isset($orderby)) {
    ...
    echo "<br>";
    OpenTable();
    $result=mysql_query("select title from $prefix"._links_categories." where cid=$cid");
    list($title) = mysql_fetch_row($result);
    echo "<center><font size=\"3\"><b>"._CATEGORY.": $title</b></font></center><br>";
    $carrytitle = $title;
    $subresult=mysql_query("select sid, title from $prefix"._links_subcategories." where cid=$cid");
    $numrows = mysql_num_rows($subresult);
    if ($numrows != 0) {
	$scount = 0;
	echo "<center><font size=\"2\">"._LALSOAVAILABLE." <i>$title</i> "._SUBCATEGORIES.":</font></center><br>"
    	    ."<table align=\"center\" border=\"0\"><tr>";
    	while(list($sid, $title) = mysql_fetch_row($subresult)) {
	    $result2 = mysql_query("select * from $prefix"._links_links." where sid=$sid");
    ...
    echo "<center><font size=\"2\">"._SORTLINKSBY.": "
        .""._TITLE." (<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=titleA\">A</a>\<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=titleD\">D</a>) "
        .""._DATE." (<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=dateA\">A</a>\<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=dateD\">D</a>) "
        .""._RATING." (<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=ratingA\">A</a>\<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=ratingD\">D</a>) "
        .""._POPULARITY." (<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=hitsA\">A</a>\<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;orderby=hitsD\">D</a>)"
	."<br><b>"._SITESSORTED.": $orderbyTrans</b></font></center><br><br>";    
    $result=mysql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where cid=$cid AND sid=0 order by $orderby limit $min,$linksperpage ");
    $fullcountresult=mysql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where cid=$cid AND sid=0");
    ...
    $linkpagesint = ($totalselectedlinks / $linksperpage);			
    $linkpageremainder = ($totalselectedlinks % $linksperpage);		
    if ($linkpageremainder != 0) {					 
    	$linkpages = ceil($linkpagesint);				
    	if ($totalselectedlinks < $linksperpage) {
    		$linkpageremainder = 0;
    	}
    } else {
    ...
    if ($linkpages!=1 && $linkpages!=0) {
        echo "<br><br>";
      	echo ""._SELECTPAGE.": ";
     	$prev=$min-$linksperpage;
     	if ($prev>=0) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=viewlink&amp;cid=$cid&amp;min=$prev&amp;orderby=$orderby&amp;show=$show\">";
    	    echo " &lt;&lt; "._PREVIOUS."</a> ]</b> ";
  	}	    	
    	$counter = 1;
 	$currentpage = ($max / $linksperpage);
       	while ($counter<=$linkpages ) {
      	    $cpage = $counter;
      	    $mintemp = ($linksperpage * $counter) - $linksperpage;
      	    if ($counter == $currentpage) {
		echo "<b>$counter</b>&nbsp";
	    } else {
		echo "<a href=\"links.php?op=viewlink&amp;cid=$cid&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show\">$counter</a> ";
	    }
       	    $counter++;
       	}      	
     	$next=$min+$linksperpage;
     	if ($x>=$linksperpage) {
    		echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=viewlink&amp;cid=$cid&amp;min=$max&amp;orderby=$orderby&amp;show=$show\">";
    		echo " "._NEXT." &gt;&gt;</a> ]</b> ";
     	}
    }
    echo "</td></tr></table>";
...
function viewslink($sid, $min, $orderby, $show) {
    ...
    $result = mysql_query("select cid, title from $prefix"._links_subcategories." where sid=$sid");
    list($cid, $stitle) = mysql_fetch_row($result);
    
    $result2 = mysql_query("select cid, title from $prefix"._links_categories." where cid=$cid");
    list($cid, $title) = mysql_fetch_row($result2);
    
    echo "<center><font size=\"3\"><b><a href=\"links.php\">"._MAIN."</a> / <a href=\"links.php?op=viewlink&amp;cid=$cid\">$title</a> / $stitle</b></font></center>";
    $orderbyTrans = convertorderbytrans($orderby);
    echo "<br><center><font size=\"2\">"._SORTLINKSBY.": "
	.""._TITLE." (<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=titleA\">A</a>\<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=titleD\">D</a>)"
	." "._DATE." (<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=dateA\">A</a>\<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=dateD\">D</a>)"
	." "._RATING." (<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=ratingA\">A</a>\<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=ratingD\">D</a>)"
        ." "._POPULARITY." (<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=hitsA\">A</a>\<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;orderby=hitsD\">D</a>)"
	."<br><b>"._SITESSORTED.": $orderbyTrans</b></font></center><br><br>";
    $result=mysql_query("select lid, url, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where sid=$sid order by $orderby limit $min,$linksperpage");
    $fullcountresult=mysql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where sid=$sid");
    $totalselectedlinks = mysql_num_rows($fullcountresult);
    echo "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"10\" border=\"0\"><tr><td><font size=\"2\">";
    $x=0;
	...
	echo "<br><br>"
        ...
        $prev=$min-$linksperpage;
        if ($prev>=0) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=viewslink&amp;sid=$sid&amp;min=$prev&amp;orderby=$orderby&amp;show=$show\">"
    		." &lt;&lt; "._PREVIOUS."</a> ]</b> ";
      	} 	
        $counter = 1;
        $currentpage = ($max / $linksperpage);
		...
		echo "<a href=\"links.php?op=viewslink&amp;sid=$sid&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show\">$counter</a> ";
	    }
            $counter++; 	
        }    	
        $next=$min+$linksperpage;
        if ($x>=$linksperpage) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=viewslink&amp;sid=$sid&amp;min=$max&amp;orderby=$orderby&amp;show=$show\">"
    		." "._NEXT." &gt;&gt;</a> ]</b> ";
        }
    }
    echo "</td></tr></table>";
...
function categorynewlinkgraphic($cat) {
    global $prefix;
    $newresult = mysql_query("select date from $prefix"._links_links." where cid=$cat order by date desc limit 1");
...
function visit($lid) {
    global $prefix;
    mysql_query("update $prefix"._links_links." set hits=hits+1 where lid=$lid");
    $result = mysql_query("select url from $prefix"._links_links." where lid=$lid");
...
function search($query, $min, $orderby, $show) {   
    ...
    $result = mysql_query("select lid, cid, sid, title, url, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where title LIKE '%$query%' OR description LIKE '%$query%' ORDER BY $orderby LIMIT $min,$linksresults");
    $fullcountresult=mysql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where title LIKE '%$query%' OR description LIKE '%$query%' ");
    $totalselectedlinks = Mysql_num_rows($fullcountresult);    
    $nrows  = mysql_num_rows($result);
    $resultx = mysql_query("select * from $prefix"._links_subcategories." where title LIKE '%$query%' ORDER BY title DESC");
    ...
    if ($query != "") {
		...
		$result2 = mysql_query("select cid, sid, title from $prefix"._links_subcategories." where title LIKE '%$query%' ORDER BY title DESC");
		...
		while(list($cid, $sid, $stitle) = mysql_fetch_row($result2)) {
		    $cate = mysql_query("select title from $prefix"._links_categories." where cid=$cid");
		    list($ctitle) = mysql_fetch_row($cate);
		    $res = mysql_query("select * from $prefix"._links_links." where sid=$sid");
		    $numrows = mysql_num_rows($res);
		    $ctitle = ereg_replace($query, "<b>$query</b>", $ctitle);
		    $stitle = ereg_replace($query, "<b>$query</b>", $stitle);
		    echo "<strong><big>&middot;</big></strong>&nbsp;<a href=\"links.php?op=viewslink&amp;sid=$sid\">$ctitle / $stitle</a> ($numrows)<br>";
		}
	echo "<br><table width=\"100%\" bgcolor=\"$bgcolor2\"><tr><td><font size=\"3\"><b>"._LINKS."</b></font></td></tr></table>";
    	$orderbyTrans = convertorderbytrans($orderby);
    	echo "<br><font size=\"2\">"._SORTLINKSBY.": "
    	    .""._TITLE." (<a href=\"links.php?op=search&amp;query=$query&amp;orderby=titleA\">A</a>\<a href=\"links.php?op=search&amp;query=$query&amp;orderby=titleD\">D</a>)"
    	    .""._DATE." (<a href=\"links.php?op=search&amp;query=$query&amp;orderby=dateA\">A</a>\<a href=\"links.php?op=search&amp;query=$query&amp;orderby=dateD\">D</a>)"
    	    .""._RATING." (<a href=\"links.php?op=search&amp;query=$query&amp;orderby=ratingA\">A</a>\<a href=\"links.php?op=search&amp;query=$query&amp;orderby=ratingD\">D</a>)"
    	    .""._POPULARITY." (<a href=\"links.php?op=search&amp;query=$query&amp;orderby=hitsA\">A</a>\<a href=\"links.php?op=search&amp;query=$query&amp;orderby=hitsD\">D</a>)"
	    ...
	    $result4 = mysql_query("select title from $prefix"._links_subcategories." where sid=$sid");
	...
	if ($prev>=0) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=search&amp;query=$query&amp;min=$prev&amp;orderby=$orderby&amp;show=$show\">"
    		." &lt;&lt; "._PREVIOUS."</a> ]</b> ";
      	}
	$counter = 1;
        ...
        while ($counter<=$linkpages ) {
    	    $cpage = $counter;
            $mintemp = ($linksperpage * $counter) - $linksresults;
            if ($counter == $currentpage) {
		echo "<b>$counter</b> ";
	    } else {
		echo "<a href=\"links.php?op=search&amp;query=$query&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show\">$counter</a> ";
	    }
            $counter++; 	
        }    	
        $next=$min+$linksresults;
        if ($x>=$linksperpage) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"links.php?op=search&amp;query=$query&amp;min=$max&amp;orderby=$orderby&amp;show=$show\">"
    		." "._NEXT." &gt;&gt;</a> ]</b>";
        }
    }
...
function viewlinkeditorial($lid, $ttitle) {
    ...
    $result=mysql_query("SELECT adminid, editorialtimestamp, editorialtext, editorialtitle FROM $prefix"._links_editorials." WHERE linkid = $lid");
    ...
    echo "<br>";
    OpenTable();
    echo "<center><font size=\"3\"><b>"._LINKPROFILE.": $displaytitle</b></font><br>";
...
function detecteditorial($lid, $ttitle) {
    global $prefix;
    $resulted2 = mysql_query("select adminid from $prefix"._links_editorials." where linkid=$lid");
    $recordexist = Mysql_num_rows($resulted2);
    if ($recordexist != 0) {
	echo " | <a href=\"links.php?op=viewlinkeditorial&amp;lid=$lid&amp;ttitle=$ttitle\">"._EDITORIAL."</a>";
    }
}
...
function viewlinkcomments($lid, $ttitle) {
    ...
    menu(1);
    echo "<br>";
    $result=mysql_query("SELECT ratinguser, rating, ratingcomments, ratingtimestamp FROM $prefix"._links_votedata." WHERE ratinglid = $lid AND ratingcomments != '' ORDER BY ratingtimestamp DESC");
    $totalcomments = mysql_num_rows($result);  
    $transfertitle = ereg_replace ("_", " ", $ttitle);
    $displaytitle = $transfertitle;
    OpenTable();
    echo "<center><font size=\"3\"><b>"._LINKPROFILE.": $displaytitle</b></font><br><br>";
    linkinfomenu($lid, $ttitle); 
...
function viewlinkdetails($lid, $ttitle) {
    ...
    echo "</td>"
	."</tr>"
	."<tr><td bgcolor=\"$bgcolor2\"><font size=\"2\">"._LINKRATING.": $avgRU</font></td></tr>"
    ...
    echo "</td>"
	."</tr>"
	."<tr><td bgcolor=\"$bgcolor2\"><font size=\"2\">"._LINKRATING.": $avgAU</font></td></tr>"
	...
	echo "</td>"
	    ."</tr>"
	    ."<tr><td bgcolor=\"$bgcolor2\"><font size=\"2\">"._LINKRATING.": $avgOU</font></td></tr>"
...
function linkfooter($lid,$ttitle) {
    echo "<font size=\"2\">[ <a href=\"links.php?op=visit&amp;lid=$lid\" target=\"_blank\">"._VISITTHISSITE."</a> | <a href=\"links.php?op=ratelink&amp;lid=$lid&amp;ttitle=$ttitle\">"._RATETHISSITE."</a> ]</font><br><br>";
    linkfooterchild($lid);
}
...
function linkfooterchild($lid) {
    include("config.php");
    if ($useoutsidevoting = 1) { 
	echo "<br><font size=\"2\">"._ISTHISYOURSITE." <a href=\"links.php?op=outsidelinksetup&amp;lid=$lid\">"._ALLOWTORATE."</a></font>";
    }
}
...
function outsidelinksetup($lid) {
    ...
    "._PROMOTE02."<br><br>
    <center><a href=\"$nukeurl/links.php?op=ratelink&amp;lid=$lid\">"._RATETHISSITE." @ $sitename</a></center><br><br>
    <center>"._HTMLCODE1."</center><br>
    <center><i>&lt;a href=\"$nukeurl/links.php?op=ratelink&lid=$lid\"&gt;"._RATETHISSITE."&lt;/a&gt;</i></center>
    <br><br>
    ...
    &lt;form action=\"$nukeurl/links.php\" method=\"post\"&gt;<br>\n
    ...
    <center>
    <form method=\"post\" action=\"$nukeurl/links.php\">
    ...
    <blockquote><i>
    &lt;form method=\"post\" action=\"$nukeurl/links.php\"&gt;<br>
...
function brokenlink($lid) {
    ...
    echo "<input type=\"hidden\" name=\"op\" value=\"brokenlinkS\"><input type=\"submit\" value=\""._REPORTBROKEN."\"></center></form>";
    CloseTable();
    include("footer.php");    
}
...
function brokenlinkS($lid, $modifysubmitter) {
    ...
    mysql_query("insert into $prefix"._links_modrequest." values (NULL, $lid, 0, 0, '', '', '', '$ratinguser', 1)");
    include("header.php");
    menu(1);
    echo "<br>";
    OpenTable();
...
function modifylinkrequest($lid) {
    ...
    if ($blocknow != 1) {
    	$result = mysql_query("select cid, sid, title, url, description from $prefix"._links_links." where lid=$lid");
    	echo "<center><font size=\"3\"><b>"._REQUESTLINKMOD."</b></font><br><font size=\"2\">";
...
function modifylinkrequestS($lid, $cat, $title, $url, $description, $modifysubmitter) {
	...
	echo "<br>";
	OpenTable();
	echo "<center><font size=\"2\">"._ONLYREGUSERSMODIFY."</font></center>";
	$blocknow = 1;
	CloseTable();
	include("footer.php");
    }
    if ($blocknow != 1) {
    	...
    	$url = stripslashes(FixQuotes($url));
    	$description = stripslashes(FixQuotes($description));
    	mysql_query("insert into $prefix"._links_modrequest." values (NULL, $lid, $cat[0], $cat[1], '$title', '$url', '$description', '$ratinguser', 0)");
    	include("header.php");
	menu(1);
	echo "<br>";
	OpenTable();
...
function addrating($ratinglid, $ratinguser, $rating, $ratinghost_name, $ratingcomments) {	
    ...
    $results3 = mysql_query("SELECT title FROM $prefix"._links_links." WHERE lid=$ratinglid");	   
    ...
    if ($ratinguser != $anonymous && $ratinguser != "outside") {
    	$result=mysql_query("select submitter from $prefix"._links_links." where lid=$ratinglid");
    ...
    if ($ratinguser!=$anonymous && $ratinguser != "outside") {
    	$result=mysql_query("select ratinguser from $prefix"._links_votedata." where ratinglid=$ratinglid");
    	while(list($ratinguserDB)=mysql_fetch_row($result)) {
    	    if ($ratinguserDB==$ratinguser) {
    	        $error = "regflood";
                completevote($error);
    ...
    if ($ratinguser==$anonymous){
    	...
    	$result=mysql_query("select * FROM $prefix"._links_votedata." WHERE ratinglid=$ratinglid AND ratinguser='$anonymous' AND ratinghostname = '$ip' AND TO_DAYS(NOW()) - TO_DAYS(ratingtimestamp) < $anonwaitdays");
    ...
    if ($ratinguser=="outside"){
    	...
    	$ytsDB = Date("Y-m-d H:i:s", $yesterdaytimestamp);
    	$result=mysql_query("select * FROM $prefix"._links_votedata." WHERE ratinglid=$ratinglid AND ratinguser='outside' AND ratinghostname = '$ip' AND TO_DAYS(NOW()) - TO_DAYS(ratingtimestamp) < $outsidewaitdays");
    ...
    if ($passtest == "yes") {
	 ...
	 mysql_query("INSERT into $prefix"._links_votedata." values (NULL,'$ratinglid', '$ratinguser', '$rating', '$ip', '$ratingcomments', now())");	
	...
	/*	 Running a SQL statement with your modded calc for ALL links will accomplish this. */
        $voteresult = mysql_query("select rating, ratinguser, ratingcomments FROM $prefix"._links_votedata." WHERE ratinglid = $ratinglid");
	$totalvotesDB = mysql_num_rows($voteresult);	
	include ("voteinclude.php");     
        mysql_query("UPDATE $prefix"._links_links." SET linkratingsummary=$finalrating,totalvotes=$totalvotesDB,totalcomments=$truecomments WHERE lid = $ratinglid");
        $error = "none";
        completevote($error);
    }
...
function completevotefooter($lid, $ttitle, $ratinguser) {
    ...
    $result=mysql_query("select url FROM $prefix"._links_links." WHERE lid=$lid");
    list($url)=mysql_fetch_row($result);
    echo "<font size=\"2\">"._THANKSTOTAKETIME." $sitename. "._LETSDECIDE."</font><br><br><br>";
    if ($ratinguser=="outside") {
	echo "<center><font size=\"2\">".WEAPPREACIATE." $sitename!<br><a href=\"$url\">"._RETURNTO." $ttitle</a></font><center><br><br>";
        $result=mysql_query("select title FROM $prefix"._links_links." where lid=$lid");
        list($title)=mysql_fetch_row($result);
...
function completevote($error) {
    ...
    if ($error == "anonflood") echo "<center><font size=\"3\"><b>You have already voted for this link in the past $anonwaitdays day(s).</b></font></center><br>";
    if ($error == "regflood") echo "<center><font size=\"3\"><b>Vote for a link only once.<br>All votes are logged and reviewed.</b></font></center><br>";
    if ($error == "postervote") echo "<center><font size=\"3\"><b>You cannot vote on a link you submitted.<br>All votes are logged and reviewed.</b></font></center><br>";
