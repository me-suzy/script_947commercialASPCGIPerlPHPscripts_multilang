...
if ($action=="edit") {
  ...
  <li>".makelinkcode("Read PM's for $user[username]","pm.php?s=$session[sessionhash]&action=list&userid=$userid")."</li>
	...
	if (is_array($userid)) {
		...
		if ($dowhat=="delete") {
			...
			$DB_site->query("DELETE FROM privatemessage WHERE userid IN($userids)");
				echo "okay.</p><p>Deleting Bedankomat references ....\n";
				flush();
			$DB_site->query("DELETE FROM bedankomat WHERE userid IN($userids)");
			        echo "okay.</p><p>Deleting user sessions ....</p>\n";
			        flush();
			...
			storetext('ids',$userids);
		        $redirectvars="&usergroupid=".$usergroupid."&daysprune=".$daysprune."&minposts=".$minposts."&joindate".$joindate."&order=".$order."&numusers=".$numusers;
			cpredirect("user.php?s=$session[dbsessionhash]&action=prune_updateposts&startat=0&dowhat=delete".$redirectvars,1);
			exit;
