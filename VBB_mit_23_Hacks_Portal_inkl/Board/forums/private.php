...
if ($backup!="") {
    ...
    if (is_array($privatemessage) or $allmsgs==1) {
      if($allmsgs==1){
      $cond = "privatemessage.userid='$bbuserinfo[userid]'";
      } else {
           $cond = "privatemessage.userid='$bbuserinfo[userid]' AND privatemessage.privatemessageid IN (";
           $msgids = array();
           while (list($key,$val)=each($privatemessage)) {
             array_push($msgids,verifyid("privatemessage",$key));
           }
           $cond .= join(",",$msgids);
           $cond .= ")";
        }
  ...
  WHERE $cond ORDER BY folderid");
      while($message=$DB_site->fetch_array($msgs)) {
        if ($bbuserinfo[pmfolders]) {
          $allfolders = split("\n", trim($bbuserinfo[pmfolders]));
          while (list($key,$val)=each($allfolders)) {
        ...
        } elseif ($message["folderid"]=="-1") {
          $foldername = $sentitemsname;
        } else {
          $foldername = $foldernames[$message["folderid"]];
        }
        // get the more useful of the to/from field
        if ($message[touserid]==$bbuserinfo[userid]) {
          $message[displayuserid]=$message[fromuserid];
          $message[displayusername]=$message[fromusername];
        } else {
          $message[displayuserid]=$message[touserid];
          $message[displayusername]=$message[tousername];
        }

        if ($message['displayuserid']=="") {
          $message['displayusername'] = "N/A";
        }
        if ($message["folderid"]==-1) {
          $sender = "an";
        } else {
          $sender = "von";
        }
        ...
        $message[message]=wordwrap($message[message],75,"\r\n");

        if($foldername!=$oldfolder){
          eval("\$messages .= \"".gettemplate("privbackup_folder")."\";");
